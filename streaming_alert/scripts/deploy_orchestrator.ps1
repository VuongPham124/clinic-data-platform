param(
    ...
    [string]$PubsubInvokerServiceAccountName = "streamalert-pubsub-inv"
)
...
$PubsubInvokerSa = "$PubsubInvokerServiceAccountName@$ProjectId.iam.gserviceaccount.com"
...
$pubsubInvokerSaExists = gcloud iam service-accounts list --project $ProjectId --filter "email=$PubsubInvokerSa" --format "value(email)"
if (-not $pubsubInvokerSaExists) {
    gcloud iam service-accounts create $PubsubInvokerServiceAccountName --project $ProjectId --display-name "PubSub Push Invoker" | Out-Null
}
...
gcloud iam service-accounts add-iam-policy-binding $PubsubInvokerSa --project $ProjectId --member "serviceAccount:$PubsubServiceAgent" --role "roles/iam.serviceAccountTokenCreator" | Out-Null

gcloud builds submit streaming_alert/orchestrator --project $ProjectId --tag $ImageUri

$envVars = "PROJECT_ID=$ProjectId,FIRESTORE_COLLECTION=$FirestoreCollection,BQ_TABLE_FQN=$BqTableFqn,COOLDOWN_SECONDS=$CooldownSeconds,NOTIFY_WEBHOOK_URL=$NotifyWebhookUrl,EMAIL_ENABLED=$EmailEnabledBool,EMAIL_PROVIDER=$EmailProvider,EMAIL_FROM=$EmailFrom,EMAIL_TO=$EmailTo,SMTP_HOST=$SmtpHost,SMTP_PORT=$SmtpPort,SMTP_USER=$SmtpUser,PUBSUB_AUTH_REQUIRED=true,PUBSUB_ALLOWED_SERVICE_ACCOUNTS=$PubsubInvokerSa"
...
if (-not [string]::IsNullOrWhiteSpace($SendgridSecretName) -or -not [string]::IsNullOrWhiteSpace($EmailPasswordSecretName)) {
    ...
    gcloud run deploy $ServiceName `
      ...
      --no-allow-unauthenticated | Out-Null
} else {
    gcloud run deploy $ServiceName `
      --project $ProjectId `
      --region $Region `
...
gcloud run services add-iam-policy-binding $ServiceName `
  --region $Region `
  --project $ProjectId `
  --member "serviceAccount:$PubsubInvokerSa" `
  --role "roles/run.invoker" | Out-Null

Write-Host "Deployed Cloud Run service: $ServiceName"
Write-Host "Use this service account in Pub/Sub push subscription OIDC token:"
Write-Host $PubsubInvokerSa
